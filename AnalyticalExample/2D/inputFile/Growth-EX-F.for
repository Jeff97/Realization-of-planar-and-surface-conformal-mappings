...
C
     ...
     &    SIX=6.D0)
        REAL  Lambda1,Lambda2,TotalT,Lambda1z0,Lambda1z1,Lambda2z0,
     &  Lambda2z1,DtltaL1,DtltaL2,Pi
C
C ----------------------------------------------------------------
...
C v1 v2 are in quadrant 2st and 3nd
C ----------------------------------------------------------------
C
...
C
        ...
        IF (STATEV(5) .EQ. 0) THEN
                STATEV(5)=COORDS(3)
        END IF
C Define the parameters of transformed coordinates system
C234567890123456789012345678901234567890123456789012345678901234567890
C
        TotalT=10.0
C
        DtltaG11=( EXP(COORDS(1))*Cos()
     &  -1.0)*(TIME(1)+DTIME)/TotalT
        DtltaG12=( Sqrt(1.0+Cosh(2*ASinh(2*Pi*STATEV(3))))
     &  -(2*Pi*STATEV(5)*Sqrt(1.0+Cosh(2*ASinh(2*Pi*STATEV(3)))))
     &  /(1+4*Pi**2*STATEV(3)**2)
     &  )*(TIME(1)+DTIME)/TotalT
        DtltaG21=( -1/Sqrt(2.0)-(Sqrt(2.0)*Pi*STATEV(5))
     &  /(1+4*Pi**2*STATEV(3)**2)
     &  )*(TIME(1)+DTIME)/TotalT
        DtltaG22=( Sqrt(1.0+Cosh(2*ASinh(2*Pi*STATEV(3))))
     &  +(2*Pi*STATEV(5)*Sqrt(1.0+Cosh(2*ASinh(2*Pi*STATEV(3)))))
     &  /(1+4*Pi**2*STATEV(3)**2)
     &  -1.0)*(TIME(1)+DTIME)/TotalT
...
C
        ...
        STATEV(9)=G22
C
C Reset components of A and B to be zero
C
        DO K1=1, 3
          DO K2=1, 3
            A1(K1, K2)=0.0
