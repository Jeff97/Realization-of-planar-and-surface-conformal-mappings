...
C
     ...
     &    FOUR=4.D0, SIX=6.D0, NELEMENT=82646)

!      TODO: please modify the element number [NELEMENT]

        DOUBLE PRECISION :: A1(3,3), BA1(6), BA1B(6),
     &  E0(NELEMENT), F0(NELEMENT), G0(NELEMENT), 
     &  L0(NELEMENT), M0(NELEMENT), N0(NELEMENT), 
     &  NodeCoordx(NELEMENT), NodeCoordy(NELEMENT)

        DOUBLE PRECISION :: Lambda1, Lambda2, LambdaS, 
     &  Lambda1z0, Lambda1z1, Lambda2z0, Lambda2z1,
     &  Pi, TotalT
...
C STATEV(4) - Current G11
C STATEV(5) - Current G22
C ----------------------------------------------------------------
C
...
! TODO: please modify path
     ...
     &  'Bunny\part2\E0.CSV',status="old")
        read(301,*) E0
        close(301)

        open(302,FILE='C:\Users\12872\Desktop\'//
     &  'Bunny\part2\F0.CSV',status="old")
        read(302,*) F0
        close(302)

        open(303,FILE='C:\Users\12872\Desktop\'//
     &  'Bunny\part2\G0.CSV',status="old")
        read(303,*) G0
        close(303)

        open(304,FILE='C:\Users\12872\Desktop\'//
     &  'Bunny\part2\L0.CSV',status="old")
        read(304,*) L0
        close(304)

        open(305,FILE='C:\Users\12872\Desktop\'//
     &  'Bunny\part2\M0.CSV',status="old")
        read(305,*) M0
        close(305)

        open(306,FILE='C:\Users\12872\Desktop\'//
     &  'Bunny\part2\N0.CSV',status="old")
        read(306,*) N0
        close(306)
...
! G is growth tensor
        ...
        Lambda1z0 = sqrt(0.5*E0(NOEL)+0.5*G0(NOEL))
        Lambda1z1 = -L0(NOEL)/Lambda1z0
        Lambda2z0 = Lambda1z0
        Lambda2z1 = -N0(NOEL)/Lambda1z0
        LambdaS   = -M0(NOEL)/Lambda1z0

        ! IF (NOEL .EQ. 10) THEN
        !   WRITE(*,*) Lambda1z0,Lambda1z1,Lambda2z1,LambdaS
        ! END IF
     ...
     &  -1.0)*(TIME(1)+DTIME)/TotalT
        DtltaG12 = ( STATEV(3)*LambdaS
     &  )*(TIME(1)+DTIME)/TotalT
        DtltaG21 = DtltaG12
        DtltaG22 = ( Lambda2z0 + STATEV(3)*Lambda2z1
     &  -1.0)*(TIME(1)+DTIME)/TotalT

!      test for zero growth --- passed
!         DtltaG11 = ( 1.0
!      &  -1.0)*(TIME(1)+DTIME)/TotalT
!         DtltaG12 = ( 0.0
!      &  )*(TIME(1)+DTIME)/TotalT
...
C G is growth tensor
        ...
        STATEV(8) = M0(NOEL)
        STATEV(9) = N0(NOEL)
        STATEV(10) = Sqrt(G11**2+G12**2+G21**2+G22**2+G33**2)
C
